...
from core.interface.menu.repositories import Menu, MenuRepository
...
class BaseKeyboard:
    ...
    def build_menu(buttons,
        ...
        if footer_buttons:
            ...
            else: menu.extend([[button] for button in footer_buttons])
        return menu

    async def get_button(self, slug: str) -> Button:
        return await self.button_dao.get(slug=slug)

    async def get_buttons(self, slug_list: list[str]) -> List[Button]:
        return await self.button_dao.get_buttons(slug_list)

    async def get_menu(self, slug: str) -> Menu:
        return await self.menu_dao.get(slug=slug)
...
class BaseMenu(BaseKeyboard):
    ...
    async def all_items_count(self, **kwargs: Any) -> int:
        valid_kwargs = self._filter_valid_kwargs(kwargs)
        all_items_count = await self.dao.get_all_items_count(**valid_kwargs)
    ...
    async def get_data(self, **kwargs: Any) -> list:
        if self._data:
            return self._data

        valid_kwargs = self._filter_valid_kwargs(kwargs)
        data = await self.dao.get_by_page(page_num=self.page_num, count_per_page=self.count_per_page, **valid_kwargs)
        self._data = data
        return data

    def _filter_valid_kwargs(self, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        """Фильтрует параметры, чтобы оставить только те, которые есть в модели"""
        valid_keys = {column.name for column in self.dao.model.__table__.columns}
    ...
    async def _get_header_buttons(self, **kwargs) -> Optional[List[InlineKeyboardButton]]:
        return None

    async def _get_footer_buttons(self, **kwargs) -> Optional[List[InlineKeyboardButton]]:
        return None
    ...
    async def _get_nav_buttons_paginations(self, **kwargs) -> List[InlineKeyboardButton]:
        ...
        for page_num in range(1, await self.all_pages_count(**kwargs) + 1):
            ...
            ikb = self.ikb(name, f'{self.nav_prefix}_{page_num}')
    ...
    async def is_nav_buttons(self, **kwargs) -> bool:
        """Определяет или нужны кнопки навигации"""
        all_items_count = await self.all_items_count(**kwargs)
        return self.count_per_page < all_items_count

    async def get_ikb(self, **kwargs: Any) -> InlineKeyboardMarkup:
        button_list = await self.get_button_list(**kwargs)
        menu = self.build_menu(
            button_list, self.columns,
            header_buttons=await self._get_header_buttons(**kwargs),
            footer_buttons=await self._get_footer_buttons(**kwargs)
        )

        if await self.is_nav_buttons(**kwargs):
            menu.insert(-1, await self.get_nav_buttons(**kwargs))
